...
public class TinyToysSceneBuilder : EditorWindow
{
    ...
    {
        Spr("Floor",   null, new Vector3(0,-1f,1f),  new Vector3(18f,8f,1f),   new Color(0.10f,0.11f,0.14f));
        Spr("Ceiling", null, new Vector3(0,4.5f,1f), new Vector3(18f,0.4f,1f), new Color(0.08f,0.09f,0.12f));
        Spr("Belt",    null, new Vector3(0,0.2f,0.5f), new Vector3(4.2f,0.55f,1f), Belt);
        for (int i = -2; i <= 2; i++)
            Spr("BeltStripe", null, new Vector3(i*0.9f,0.2f,0.3f), new Vector3(0.12f,0.48f,1f), new Color(0.45f,0.47f,0.52f));
    }
    ...
    static void MakeFactory()
    {
        ...
        var mA = MachinePrefab("MachineA_Assembly", Machine.MachineType.AssemblyA, new Vector3(-3.3f,0.5f,0f), MachineA);
        var mB = MachinePrefab("MachineB_Paint",    Machine.MachineType.PaintPackB, new Vector3(3.3f,0.5f,0f),  MachineB);
    ...
    static void MakeWorkers()
    {
        ...
        for (int i = 0; i < 3; i++)
            WorkerPrefab($"Worker_0{i+1}", new Vector3(xs[i],-2.6f,0f)).transform.SetParent(root.transform);
    }
    ...
    static void MakeUI()
    {
        ...
        // Resources (left)
        var resPanel   = Panel("ResPanel",  hud, 0.01f, 0.15f, 0.16f, 0.85f);
        var woodText   = TMP("WoodText",   resPanel, "Wood: 50/100",   16, new Color(0.8f,0.95f,0.55f)); Anch(woodText.gameObject,   0.05f,0.76f,0.95f,1f);
        var paintText  = TMP("PaintText",  resPanel, "Paint: 50/100",  16, new Color(0.95f,0.65f,0.90f)); Anch(paintText.gameObject,  0.05f,0.52f,0.95f,0.76f);
        var powerText  = TMP("PowerText",  resPanel, "Power: 100/100", 16, new Color(0.95f,0.90f,0.35f)); Anch(powerText.gameObject,  0.05f,0.28f,0.95f,0.52f);
        var workerText = TMP("WorkerText", resPanel, "Workers: 3/3",   16, new Color(0.55f,0.85f,1.0f));  Anch(workerText.gameObject, 0.05f,0.04f,0.95f,0.28f);
        ...
        var pauseBtn = Btn("PauseBtn",        hud, "Pause",          new Color(0.2f,0.2f,0.35f)); Anch(pauseBtn, 0.84f, 0.01f, 0.99f, 0.08f);
        ...
        var orderBoardPanel = Panel("OrderBoardPanel", hud, 0.76f, 0.08f, 1f, 0.76f, new Color(0.06f, 0.09f, 0.17f, 0.92f));
        ...
        // Slots container (vertical layout group expected in runtime)
        var orderSlotsCont = Rect("OrderSlotsContainer", orderBoardPanel);
        Anch(orderSlotsCont, 0.02f, 0f, 0.98f, 0.90f);
        var vlg = orderSlotsCont.AddComponent<UnityEngine.UI.VerticalLayoutGroup>();
        vlg.childForceExpandHeight = false;
        vlg.spacing = 5f;
        vlg.padding = new RectOffset(5, 5, 5, 5);
        
        var obUI = uiMgrGO.AddComponent<OrderBoardUI>();
        obUI.slotsContainer = orderSlotsCont.transform;
        
        // Generate and assign the slot prefab
        GameObject slotPrefab = MakeOrderSlotPrefab();
        obUI.slotPrefab = slotPrefab;
        ...
        var shopToggleBtn = Btn("ShopToggleHUDBtn", hud, "Shop [Tab]", new Color(0.15f,0.20f,0.38f)); Anch(shopToggleBtn, 0.84f, 0.08f, 1f, 0.16f);
    ...
    static GameObject MachinePrefab(string n, Machine.MachineType t, Vector3 pos, Color col)
    {
        ...
        var light = Spr("StatusLight", root, new Vector3(0.65f,0.95f,-0.3f), new Vector3(0.22f,0.22f,1f), Color.green);
    ...
    // ═══════════════════════════════════════════════════════════════════════
    static GameObject MakeOrderSlotPrefab()
    {
        string folderPath = "Assets/Prefabs/UI";
        string filePath = $"{folderPath}/OrderSlot.prefab";
        ...
        // Always recreate it to ensure latest hierarchy
        var root = new GameObject("OrderSlot", typeof(RectTransform));
        root.layer = 5; // Layer UI
        var rt = root.GetComponent<RectTransform>();
        rt.sizeDelta = new Vector2(300, 80); // Exact dimensions for VLG
        
        var img = root.AddComponent<Image>();
        img.color = new Color(0.12f, 0.15f, 0.22f, 0.95f); // Slightly lighter than panel

        var nameTxt = TMP("OrderNameText", root, "Order Name", 16, UI_Orange, TextAlignmentOptions.TopLeft);
        Anch(nameTxt.gameObject, 0.05f, 0.70f, 0.95f, 0.95f);

        var reqTxt = TMP("RequiredText", root, "  • Toy ×1", 14, Color.white, TextAlignmentOptions.Left);
        Anch(reqTxt.gameObject, 0.05f, 0.25f, 0.65f, 0.65f);

        var barBg = Img("DeadlineBarBg", root, new Color(0.1f, 0.1f, 0.1f));
        Anch(barBg.gameObject, 0.05f, 0.05f, 0.95f, 0.15f);

        var barFill = Img("DeadlineBar", root, UI_Green);
        Anch(barFill.gameObject, 0f, 0f, 1f, 1f); // Anchored to barBg
        barFill.gameObject.transform.SetParent(barBg.transform, false);
        barFill.type = Image.Type.Filled;
        barFill.fillMethod = Image.FillMethod.Horizontal;
        barFill.fillAmount = 1f;

        var btn = Btn("DeliverButton", root, "Deliver", UI_Blue);
        Anch(btn, 0.70f, 0.25f, 0.95f, 0.65f);

        // Save layout element to force VLG to respect size
        ...
        Debug.Log($"[SceneBuilder] Created OrderSlot prefab at {filePath}");
        return prefab;
    }
}
#endif
